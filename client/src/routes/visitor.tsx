...
import type { Innings, MatchState } from '@/lib/cricket/types';
...
function VisitorView() {
  ...
  const teams = hasLive ? liveState!.teams : [{ name: '', players: [] }, { name: '', players: [] }];
  ...
  const target = liveState?.currentInningsIdx === 1 && liveState.innings.length > 0 ? inningsTotals(liveState.innings[0]).runs + 1 : null;
  const runsNeeded = target !== null && totals ? Math.max(0, target - totals.runs) : null;
  const ballsRemaining = target !== null && totals ? Math.max(0, overs * 6 - totals.legalBalls) : null;
  const rrr = target !== null && ballsRemaining && ballsRemaining > 0 && runsNeeded !== null ? (runsNeeded / ballsRemaining) * 6 : 0;
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <Link to="/history" className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm">History</Link>
            {getAuth() && !state.authUser ? (
              ...
              <Link to="/" className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm">Back</Link>
            )}
          </div>
        </div>

        {!hasLive ? (
          <div className="glass-card p-6 text-sm text-muted-foreground">No live match currently.</div>
        ) : (
          <>
            <div className="sticky top-4 z-10 rounded-3xl border border-white/10 bg-[rgba(255,255,255,0.03)] p-6 shadow-lg backdrop-blur-xl">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-wider text-muted-foreground">{battingTeam?.name} — {liveState!.currentInningsIdx === 0 ? '1st' : '2nd'} innings</p>
                  <div className="mt-2 flex flex-wrap items-baseline gap-3">
                    <span className="text-5xl font-bold text-gold">{totals!.runs}/{totals!.wickets}</span>
                    <span className="text-sm text-muted-foreground">({oversString(totals!.legalBalls)}/{overs})</span>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3 text-right text-xs sm:grid-cols-4">
                  ...
                  {target !== null && runsNeeded !== null && ballsRemaining !== null && (
                    <>
                      ...
                      <div>
                        ...
                        <p className="font-semibold">{runsNeeded} off {ballsRemaining}</p>
                      </div>
                      <div>
                  ...
                  <div>
                    ...
                    <p className="font-semibold">{partnership.runs}({partnership.balls})</p>
                  </div>
                </div>
              </div>

              <div className="mt-4 flex flex-wrap gap-2 text-xs">
                <Chip>👤 {battingTeam?.players.find((p) => p.id === inn!.strikerId)?.name ?? '—'} *</Chip>
                <Chip>{battingTeam?.players.find((p) => p.id === inn!.nonStrikerId)?.name ?? '—'}</Chip>
                <Chip>🎯 {bowlingTeam?.players.find((p) => p.id === inn!.bowlerId)?.name ?? '—'}</Chip>
                ...
                {previousInnings.map((prev, index) => {
                  ...
                  return (
                    <div key={index} className="space-y-4">
                      <div className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Previous Innings — {team.name}</div>
...
function Chip({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs">{children}</span>
  );
}

function BallBadge({ b }: { b: any }) {
  let label = '';
  ...
  return (
    <span className={`inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold ${cls}`}>{label}</span>
  );
}

function BattingScorecard({ inn, team }: { inn: Innings; team: { name: string; players: { id: string; name: string }[] } }) {
  ...
  return (
    <section className="glass-card overflow-hidden">
      <div className="border-b border-white/10 px-5 py-3">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Batting — {team.name}</h3>
      </div>
...
function BowlingScorecard({ inn, team }: { inn: Innings; team: { name: string; players: { id: string; name: string }[] } }) {
  ...
  return (
    <section className="glass-card overflow-hidden">
      <div className="border-b border-white/10 px-5 py-3">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Bowling — {team.name}</h3>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="text-xs uppercase text-muted-foreground">
